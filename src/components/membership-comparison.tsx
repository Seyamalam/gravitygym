  ...
  return (
    <section className="py-20 bg-black" id="membership-comparison">
      <div className="container mx-auto px-4 md:px-8">
          ...
          {membershipPlans.map((plan, index) => (
            <CardContainer key={index} className="w-full">
              <CardBody className="w-full h-full flex flex-col bg-zinc-800 border border-zinc-700 rounded-xl overflow-hidden shadow-lg">
                <CardItem
                  translateZ="50"
                  className={`w-full text-center px-6 pt-8 pb-4 ${plan.popular ? "bg-lime-500/10" : ""}`}
                >
                  {plan.popular && (
                    <div className="bg-lime-500 text-black text-sm font-bold py-1 px-4 rounded-full mb-4 inline-block shadow">
                      MOST POPULAR
                    </div>
                  )}
                  <h3 className="text-2xl font-extrabold text-white drop-shadow mb-2">{plan.name}</h3>
                  ...
                  <p className="text-gray-100 font-medium">{plan.description}</p>
                  {plan.promotion && (
                    <div className="mt-4 bg-lime-500/20 text-lime-400 p-2 rounded-lg text-sm font-semibold">
                  ...
                  <ul className="space-y-3">
                    {plan.features.map((feature, featureIndex) => (
                      <li key={featureIndex} className="flex items-start">
                        ...
                        <span className={feature.included ? "text-white font-medium" : "text-zinc-500"}>
                          {feature.feature}
                        </span>
                      </li>
                    ))}
                ...
                <CardItem translateZ="100" className="px-6 pb-8 pt-2">
                  <Button className={`w-full ${plan.popular ? "bg-lime-500 hover:bg-lime-600" : "bg-zinc-700 hover:bg-zinc-600"} text-black font-extrabold text-lg py-4 shadow-lg`}>
                    SELECT PLAN
                  </Button>
                </CardItem>
              </CardBody>
        ...
        <div className="mt-12 bg-zinc-800/50 border border-zinc-700 rounded-xl p-6 max-w-4xl mx-auto shadow-lg">
          <h3 className="text-xl font-extrabold mb-4 text-center text-white drop-shadow">ALL MEMBERSHIPS INCLUDE</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <div className="flex items-center">
              <Check className="h-5 w-5 text-lime-500 mr-3" />
              <span className="text-white font-medium">Free WiFi</span>
            </div>
            <div className="flex items-center">
              <Check className="h-5 w-5 text-lime-500 mr-3" />
              <span className="text-white font-medium">Shower Facilities</span>
            </div>
            <div className="flex items-center">
              <Check className="h-5 w-5 text-lime-500 mr-3" />
              <span className="text-white font-medium">Free Parking</span>
            </div>
            <div className="flex items-center">
              <Check className="h-5 w-5 text-lime-500 mr-3" />
              <span className="text-white font-medium">Towel Service</span>
            </div>
            <div className="flex items-center">
              <Check className="h-5 w-5 text-lime-500 mr-3" />
              <span className="text-white font-medium">Fitness App Access</span>
            </div>
            <div className="flex items-center">
              <Check className="h-5 w-5 text-lime-500 mr-3" />
              <span className="text-white font-medium">No Contract</span>
            </div>
          </div>
        </div>
      </div>
